...
pub(super) fn generate(
    ...
    let cheek_columns = detail.segments(12, 2);
    let mut surface = Surface::new(detail, detail.segments(AROUND, 16).next_multiple_of(8));
    let around = surface.around;
    let opening_start = around / 8;
    let eye = d.eye_opening.radians();
    ...
    if corner > 1e-6 {
        let rows = detail.segments(4, 2);
        heights.extend((1..=rows).map(|i| shelf - corner * i as f32 / rows as f32));
    }
    let start = shelf - corner;
    let rows = detail.segments(8, 2);
    heights.extend((1..=rows).map(|i| start + (lower - start) * i as f32 / rows as f32));
