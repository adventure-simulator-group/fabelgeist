...
All leaf recipe IDs use one CPU field evaluator on native and WASM. Species select
`LeafParameters` data from `TextureParameters.leaves`; they never select a shape
algorithm. The 38 shape presets include the reference catalog plus hazel,
blackthorn and hawthorn. Living and dry oak share a shape with different palette
and relief data.
...
`LeafShape::interpolate` blends the actual structural controls and preserves exact
...
antialiasing. Height and normal detail are separate from pigment. A filtered vein
mask drives rounded vein relief; dome, curl, corrugation and tissue controls add
surface relief. Both normal maps derive from their own height field, reversing
the back map's green channel for the back-face tangent frame.
