...
impl PreparedResident {
    fn new(draft: &ResidentDraft) -> Result<Self, String> {
        ...
        let household = format!(
            "the {} {}",
            SURNAMES[resident_random(&draft.seed, ResidentEntropyStream::HouseholdName)
                .index(SURNAMES.len())],
...
fn insert_identity(
    ...
    insert_persistent_npc_character(
        ctx,
        resident_name(&draft.seed, draft.sex == Sex::Female),
        draft.character_id(),
        settlement_id,
        prepared.stable_seed,
        None,
...
fn insert_profile(
    ...
    ctx.db
        ...
        .insert(SettlementResidentProfile {
            ...
            household: prepared.household.clone(),
