// Browser file and worker boundary; all authoring and generation are shared Rust.
const storageKey = 'fabelgeist.heraldry.document';
let pendingImport;
export function load_saved() { try { return localStorage.getItem(storageKey); } catch { return undefined; } }
export function save_document(source) { try { localStorage.setItem(storageKey, source); } catch (error) { return `Autosave unavailable: ${error.message}`; } }
