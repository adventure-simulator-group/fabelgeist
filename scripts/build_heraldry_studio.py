#!/usr/bin/env python3
"""Build a self-contained static Heraldry Studio, independently of the strategic website."""
...
CRATE = ROOT / "crates/adventuresim-heraldry-studio"

def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--output", type=Path, default=ROOT / "target/heraldry-studio/site")
    ...
    command = ["cargo", "build", "--locked", "-p", "adventuresim-heraldry-studio", "--lib", "--target", "wasm32-unknown-unknown"]
    if not args.dev:
    ...
    if not target.is_absolute():
        target = ROOT / target
    wasm = target / "wasm32-unknown-unknown" / ("debug" if args.dev else "release") / "adventuresim_heraldry_studio.wasm"
