// Bevy 0.19.1, MIT: BEVY_LICENSE_MIT.txt. Local LUT-coordinate and ground-radius corrections.
// Bruneton-derived segment sampling: BRUNETON_LICENSE.txt.
...
fn unit_to_sub_uvs(val: vec2<f32>, resolution: vec2<f32>) -> vec2<f32> {
    return (val * (resolution - 1.0f) + 0.5f) / resolution;
}
