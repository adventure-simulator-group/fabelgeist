// Bevy 0.19.1, MIT: BEVY_LICENSE_MIT.txt. Per-sample atmosphere transport.
...
fn main(
...
#ifdef MULTISAMPLED
    let depth = textureLoad(depth_texture, vec2<i32>(in.position.xy), i32(sample_index));
#else
    let depth = textureLoad(depth_texture, vec2<i32>(in.position.xy), 0);
#endif
    // Match scattering and transmission to the color/depth coverage sample.
    ...
    if depth == 0.0 {
        ...
        let t = ndc_to_camera_dist(vec3(uv_to_ndc(uv), depth));
        inscattering = sample_aerial_view_lut(uv, t);
        transmittance = sample_transmittance_lut_segment(r, mu, t);
        if should_raymarch {
            let result = raymarch_atmosphere(world_pos, ray_dir_ws, t, max_samples, uv, false);
            inscattering = result.inscattering;
            transmittance = result.transmittance;
        }
    }
